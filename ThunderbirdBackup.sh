...
if [ "$1" != "force" ]; then
	...
	LAST_BACKUP_FILE=$(find $BACKUP_FOLDER -type f -print0 | xargs -0 ls -ltd | awk '{print $(NF)}' | head -n 1)
...
for i in {1..$ATTEMPTS_TO_STOP_THUNDERBIRD}
do
...
then
	zenity --error --text "Can't do backup - Thunderbird still running.."
	exit
fi
...
FILES_LIST=$(find $BACKUP_FOLDER -type f -print0 | xargs -0 ls -ltd | awk '{print $(NF)}')
